...
#'
#' @param x a [`terra::SpatRaster`], `stars` or [`terra::SpatRasterDataset`] to
#'   clamp.
#' @param training the training dataset (a [`data.frame`] or a [`sf::sf`]
#'   object.
#' @param .col the column containing the presences (optional). If specified, it
#'   is excluded from the clamping.
#' @param use_na a boolean determining whether values outside the range of the
#'   training dataset are removed (set to NA). If FALSE (the default), values
#'   outside the training range are replaced with the extremes of the training
#'   range.
