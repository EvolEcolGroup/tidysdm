...
#' as the reference level of the factor (as expected by other classification models).
...
#'     ref="presence"))
#'   mod3 <- maxnet_fit(presence~.,data=bradypus_tb, classes="lq")
...
maxnet_fit <- function(formula, data, regmult=1.0, classes="default",
  ...
  maxnet_obj<-maxnet::maxnet(p=response,
                 data=predictors,
                 f=maxnet::maxnet.formula(p = response,
                                          data = predictors,
                                          classes = classes),
                 regmult=regmult,
                 regfun=regfun,
...
maxnet_predict <- function(object, newdata, type=c("class", "prob"),
  ...
  prob_vect <- stats::predict(object,newdata=as.data.frame(newdata), type=maxnet_type, clamp=clamp)
  if (type=="class"){
