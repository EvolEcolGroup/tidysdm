...
#' each time step (i.e. a vector of length equal to the number of time steps in raster)
...
sample_background_time <- function(data, raster, n_per_time_step, coords = NULL, time_col = "time",
  ...
  for (i_index in seq_len(terra::nlyr(raster))) {
    ...
    # this is really only needed for dist_max; for other methods, min and max time indeces are the same thing
    ...
    if (nrow(data_sub)==0){
      if (method[1]=="dist_max"){
        stop("for time ", time_steps[i_index]," there no presences when ", n_per_time_step[i_index],
             " background points were requested; it is not possible to sample from a 'dist_max' buffer without presences!")
      } else {
  ...
  if (return_pres){
    ...
    background <- background %>% dplyr::bind_rows(presences) %>%
