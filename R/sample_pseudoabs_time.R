...
#' see [sample_background_time]). The following methods are implemented:
...
#' `Date` format, or use `tstep="years"'. See [terra::time()] for details on how
...
#'   NAs.
#' @param n_per_presence number of pseudoabsence points to sample for each
#'   presence
...
#'   their effect when sampling pseudoabsences. If set to zero, presences have
#'   an effect only on the time step to which they are assigned in `raster`; if
#'   a positive value, it defines the number of days before and after the date
#'   provided in the `time` column for which the presence should be considered
#'   (e.g. 20 days means that a presence is considered in all time steps
#'   equivalent to plus and minus twenty days from its date).
