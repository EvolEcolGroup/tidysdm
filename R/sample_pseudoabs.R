                              ...
                              return_pres = TRUE)
{
  ...
  if (length(cell_id)>n){
    ...
    warning("There are fewer available cells for raster '",time(sampling_raster),"' (",nrow(xy_pres)," presences) than the requested ", n, " points. Only ", length(cell_id), " will be returned.\n")
  }
