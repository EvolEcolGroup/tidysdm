test_that("simple_ensemble predictions", {
  ...
  mean_pred_members <- predict(test_ens, new_data = two_class_dat, members=TRUE)
  ...
  pred_thresh <- predict(test_ens, new_data = two_class_dat, fun="none", metric_thresh = c("roc_auc",0.88))
  expect_true(ncol(pred_thresh)==(ncol(mean_pred)*2))
  pred_thresh_m <- predict(test_ens, new_data = two_class_dat, fun="mean", metric_thresh = c("roc_auc",0.88))
  ...
  pred_thresh_1 <- predict(test_ens, new_data = two_class_dat, fun="none", metric_thresh = c("accuracy",0.815))
  expect_true(ncol(pred_thresh_1)==(ncol(mean_pred)))
  pred_thresh_1m <- predict(test_ens, new_data = two_class_dat, fun="mean", metric_thresh = c("accuracy",0.815))
  expect_true(ncol(pred_thresh_1m)==(ncol(mean_pred)))
  # and throw and error if we end up without any model!
  expect_error(predict(test_ens, new_data = two_class_dat, fun="mean", metric_thresh = c("accuracy",0.83)),
  ...
  pred_class <- predict(test_ens, new_data = two_class_dat, type="class", class_thresh = c("tss_max"),
                        fun=c("mean", "median"))
