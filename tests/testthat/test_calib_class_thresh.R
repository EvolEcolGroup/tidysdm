...
test_that("calibrate class thresholds for simple_ensemble", {
  ...
  # the names of the three elements should be the same as the names of the three repeats
  expect_true(
    all(names(attr(lacerta_rep_ens, "class_thresholds_list")) %in%
  ...
  # now check that the class thresholds are the same as those from a simple_ensemble
  test_ens <- calib_class_thresh(test_ens, class_thresh = c("sens", 0.9))
  expect_equal(
