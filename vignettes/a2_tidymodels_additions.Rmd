...
section](#initial-split) section if you don't), we can go beyond a simple
ensemble that keeps only the single best configuration of each model type and
averages their predictions. Instead, we can build a stack ensemble using the
`stacks` package. Stacking fits a meta-learner that learns how to optimally
weight predictions from multiple candidate models, including multiple tuned
versions of the same algorithm with different hyper-parameters, so that the
final ensemble can exploit complementary strengths across model configurations.
