...
We reuse the example on the Iberian lizard from the 
[`tidysdm` overview](https://evolecolgroup.github.io/tidysdm/articles/a0_tidysdm_overview.html) article. Each subsection is written to be self-contained, so you can read and run them independently.
...
# Clean up objects created in the "Using multi-level factors as predictors" section
rm(
  lacerta_thin,
  lacerta_rec,
  lacerta_prep,
...
Some model types (e.g. GLMs and SVMs) can be sensitive to multicollinearity among predictors, while others (notably random forests) are generally more robust to correlated inputs. In this section, we create two separate recipes: one that keeps all predictors, and another that retains only a reduced set of weakly correlated variables.
...
We then create two recipes: one that keeps all variables, and another that removes highly correlated ones.
...
The standard `tidymodels` workflow begins by splitting the data into a **training set** (used to fit and tune models) and an **independent testing set** (held out for final evaluation). This step is essential for getting an honest estimate of predictive performance of any SDM, yet it has often been omitted in traditional SDM practice. This is largely for historical reasons: SDM datasets were frequently small, and holding data back for testing felt too costly. In this section, we demonstrate a robust approach by holding out 20% of the data (1/5) for testing and use the remaining 80% for training.
...
Same as in the [`tidysdm` overview](https://evolecolgroup.github.io/tidysdm/articles/a0_tidysdm_overview.html) article we can now build a `workflow_set` of different models:
...
And create an ensemble, selecting the best set of parameters for each model using Boyce continuous index as our metric:
...
Now that we have an explicit training/testing split (see the [previous section](#initial-split) section), we can go beyond a simple ensemble that keeps only the single best version of each model type. Instead, we can build a stack ensemble using the `stacks` package. Stacking fits a meta-learner that learns how to combine predictions from multiple candidate models, including multiple tuned versions of the same algorithm with different hyper-parameters, so that the final ensemble can exploit complementary strengths across model configurations.
...
lacerta_stack <-
  ...
  blend_predictions(
    metric = sdm_metric_set() # use our SDM metrics for calculating stacking coefficients
  ) %>%
...
We can see that two versions of the random forest and one of SVM were selected;
