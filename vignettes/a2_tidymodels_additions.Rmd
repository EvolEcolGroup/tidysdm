...
An issue with machine learning algorithms is that it is not easy to undestand the
role of different variables in giving the final prediction. A number of packages
have been created to explore and explain the beahviour of ML algorithms, such as those
used in `tidysdm`. Here we demonstrate how to use DALEX, an excellent package
that has a methods to deal with `tidymodels`. `tidysdm` contains some methods that
allow use to use the DALEX functions directly on `tidysdm` ensembles. Let's build
a simple ensemble:

```{r}
lacerta_thin_sub <- lacerta_thin %>% select(-c("bio01", "bio02", "bio03", "bio04", "bio07", "bio08", 
                                           "bio09", "bio10", "bio11", "bio12", "bio14", "bio16", 
                                           "bio17", "bio18", "bio19", "altitude"))
lacerta_rec <- recipe(lacerta_thin_sub, formula=class~.)
...
queried by different functions in the package. For this purpose, we use the
...
We can now make predictions with this stacked ensemble:
