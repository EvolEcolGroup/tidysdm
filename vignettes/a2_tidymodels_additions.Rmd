...
that has a methods to deal with `tidymodels`. `tidysdm` contains additional functions that
...
explanation (following the DALEX lingo). As a first setp, we use the
...
```{r}
profile_list <- lapply(explainer_list,model_profile, N = 500, variables = "bio05")
plot(profile_list)
```
...
analogous to the one that we generated in the Overview vignette:
...
climate_vars <-lacerta_rec_all$var_info %>% filter(role=="predictor") %>% pull(variable)
...
terra::gdal(warn=3)
climate_present<-terra::rast(system.file("extdata/lacerta_climate_present_10m.nc", package="tidysdm"))
climate_vars <- lacerta_rec_all$var_info %>% filter(role=="predictor") %>% pull(variable)
