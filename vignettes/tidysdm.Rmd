...
```{r cast_to_sf}
library(sf)
lacerta_sf <- st_as_sf(lacerta, coords = c("longitude", "latitude"))
st_crs(lacerta_sf) <- "+proj=longlat"
```
...
You can use the R package `crstools` (https://evolecolgroup.github.io/crstools/index.html, also available on [CRAN](https://cran.r-project.org/web/packages/crstools/index.html))
to [find an appropriate equal area projection](https://link.springer.com/chapter/10.1007/978-3-319-51835-0_9) for any region. 
...
In this case, we will use a Albers Equal Area Conic projection centered on the Iberian peninsula, with km as units. The proj4 string is:
...
```{r thin_by_cell}
set.seed(1234567)
lacerta_thin <- thin_by_cell(lacerta_proj, raster = land_mask_proj)
nrow(lacerta_thin)
```

```{r}
pres_data <- terra::extract(land_mask_proj, lacerta_thin)
summary(pres_data)
```
...
```{r thin_by_dist}
set.seed(1234567)
lacerta_dist <- thin_by_dist(lacerta_thin, dist_min = km2m(20))
nrow(lacerta_dist)
```
...
st_crs(lacertidae_background_sf) <- "+proj=longlat"
lacertidae_background <- st_transform(lacertidae_background_sf, crs = iberia_proj4)
```
...
lacertidae_background_raster <- rasterize(lacertidae_background_proj,
  land_mask_proj,
  fun = "count"
)
...
```{r sample_background}
set.seed(1234567)
lacerta_bg <- sample_background(
  data = lacerta_dist, raster = lacertidae_background_raster,
  n = 3 * nrow(lacerta_dist),
  method = "bias",
...
ggplot() +
  geom_spatraster(data = land_mask_proj, aes(fill = land_mask_1985)) +
  geom_sf(data = lacerta_bg, aes(col = class)) +
  guides(fill = "none")
```
...
```{r}
lacerta_climate <- lacerta_bg %>%
  bind_cols(terra::extract(climate_present_proj, lacerta_bg, ID = FALSE))
```


```{r echo=FALSE, results='hide', eval=create_sample_data}
terra::saveRDS(lacerta_climate, "../inst/extdata/lacerta_thin_all_vars.rds")
```
...
lacerta_climate %>% plot_pres_vs_bg(class)
```
...
```{r}
lacerta_climate %>% dist_pres_vs_bg(class)
```
...
climate_present_suggested <- climate_present_proj[[suggested_vars]]

vars_uncor <- filter_collinear(climate_present_suggested,
  cutoff = 0.7,
  method = "cor_caret"
)
vars_uncor
...
```{r}
lacerta_vars <- lacerta_climate %>% select(all_of(c(vars_uncor, "class")))
climate_present_vars <- climate_present_proj[[vars_uncor]]
names(climate_present_vars) # variables retained in the end
```
...
```{r recipe}
lacerta_rec <- recipe(lacerta_vars, formula = class ~ .)
lacerta_rec
```
...
```{r}
lacerta_vars %>% check_sdm_presence(class)
```
...
library(tidysdm)
set.seed(105)
lacerta_cv <- spatial_block_cv(lacerta_vars, v = 5)
autoplot(lacerta_cv)
```
...
prediction_present <- predict_raster(lacerta_ensemble, climate_present_vars)
ggplot() +
  geom_spatraster(data = prediction_present, aes(fill = mean)) +
  scale_fill_terrain_c() +
  # plot presences used in the model
  geom_sf(data = lacerta_vars %>% filter(class == "presence"))
```
...
prediction_present_boyce <- predict_raster(lacerta_ensemble, climate_present_vars,
...
for (i_repeat in 1:3) {
  # thin the data
  lacerta_thin_rep <- thin_by_cell(lacerta_proj, raster = climate_present_vars) 
  lacerta_thin_rep <- thin_by_dist(lacerta_thin_rep, dist_min = 20000)
  # sample pseudo-absences
  lacerta_pa <- sample_pseudoabs(lacerta_thin_rep,
    n = 3 * nrow(lacerta_thin_rep),
    raster = climate_present_vars,
    method = c("dist_min", 50000)
  )
  # get climate
  lacerta_thin_rep_climate <- lacerta_pa %>%
    bind_cols(terra::extract(climate_present_vars, lacerta_pa, ID = FALSE))
  # create folds
  lacerta_thin_rep_cv <- spatial_block_cv(lacerta_thin_rep_climate, v = 5)
  # create a recipe
  lacerta_thin_rep_rec <- recipe(lacerta_thin_rep_climate, formula = class ~ .)
...
We can summarise the goodness of fit of models for each repeat with `autoplot()` and 
`collect_metrics()` functions.
...
We can then predict in the usual way using `predict_raster`, specifying the aggregating function with the `fun` parameter. We will take the mean and median of all models (you can also use weighted alternatives), without filtering by performance, and plot the results:

```{r, fig.width=6, fig.height=4}
lacerta_rep_ens_plot <- predict_raster(lacerta_rep_ens, climate_present_vars,
  fun = c("mean", "median")
)
ggplot() +
...
If we want to convert predictions from a repeated ensemble into binary outputs, we need to calibrate the threshold for each repeat separately, as the models in each repeat are different and thus the optimal threshold may differ across repeats. We can do this with `calib_class_thresh()`, which will add a new column to the `repeat_ensemble` object with the calibrated thresholds for each repeat.
...
To combine binary predictions across repeats when predicting with `predict_raster`, we can use either the majority class (the class with the highest proportion across repeats is predicted) by specifying `class_fun = "majority"` or the proportion of the "presence" class across repeats (`class_fun = "prop"`). Note that we can also filter the individual models within each repeat with `metric_thresh`.

```{r, fig.width=6, fig.height=4}
# predict with the majority class across repeats
prediction_binary_repens <- predict_raster(lacerta_rep_ens_calib,
  climate_present_vars,
  type = "class",
  fun = "median",
  class_thresh = c("tss_max"),
...
There is currently no built-in support for variable response curves for `repeat_ensemble` objects; users can manually generate them for example by using the training data and recipe from a single repeat (similar as shown above for `simple_ensemble`), though this does not account for variation in data across repeats. A more principled solution is planned for a future release.
