...
`collect_metrics()` and visualise it with `autoplot()`.
...
TODO: check how to visualise the contribution of individual variables in repeated ensembles. The code below uses one of the thinned datsets, but datasets differ slightly for differtent repeats. Ideally, we would deal with this for example by taking means and ranges for each repeat datasets and then processing those (aplicable for this tidymodels solution) or somehow combining the datasets for each repeat or a subset of repeats, also applicable for response curves with DALEX (TODO test this with DALEX as well and find out how it works).

```{r, fig.width=6, fig.height=4}
bio05_prof <- recipe(lacerta_thin, formula = class ~ .) %>%
  step_profile(-bio05, profile = vars(bio05)) %>%
  prep(training = lacerta_thin)
...
bio05_data <- bio05_data %>%
  mutate(
    pred = predict(lacerta_rep_ens, bio05_data, fun = "mean")$mean
  )
