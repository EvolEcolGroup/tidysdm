...
In this vignette, we show how a Species Distribution Model can be fitted with
`tidysdm` on time-scattered (i.e.palaeontological, archaeozoological, archaeological) data, with samples covering different time periods.
...
```{r cast to sf, fig.width=6, fig.height=4}
library(tidyterra)
ggplot() +
...
```{r thin_by_dist}
set.seed(123)
horses<-thin_by_dist_time(horses, dist_min = km2m(100),
...
climate_full<-pastclim::region_series(bio_variables = climate_vars, 
                                      data="Example", 
...
```{r thin_by_cell}
set.seed(123)
horses<-thin_by_cell_time(horses, raster = climate_full,
                          time_col="time_bp",
                          lubridate_fun = ybp2date)
nrow(horses)
```
...
horses_models <- 
  horses_models %>% 
  workflow_map("tune_grid", resamples = horses_cv, grid = 5, 
