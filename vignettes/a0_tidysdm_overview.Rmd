...
ggplot() +
  ...
  geom_sf(data = lacerta) + scale_fill_gradient(na.value = "transparent") # removed NA (sea) grey background, if people prefer like this

```
@TODO remove that presence outliner 
...
for the species. There are two approaches, we can either sample the background
with `sample_background()`,
or we can generate pseudo-absences with `sample_pseudoabs()`. In this example,
we will sample the background; more specifically, we will attempt to account for potential
sampling biases by using a target group approach, where presences from other species
within the same taxonomic group are used to condition the sampling of the background,
providing information on differential sampling of different areas within the region
of interest.

We will start by downloading records from 8 genera
of *Lacertidae*, covering the same geographic region of the Iberian peninsula
from GBIF <https://doi.org/10.15468/dl.53js5z>:
...
We need to convert these observations into a raster whose values are the number
of records (which will be later used to determine how likely each cell is to
be used as a background point):
...
We can see that the sampling is far from random, with certain locations having 
very large number of records. We can now sample the background, using the 'bias'
method to represent this heterogeneity in sampling effort:
...
Next, we extract climate
for all presences and background_points:
...
Based on this paper
(<https://doi.org/10.1007/s10531-010-9865-2>), we are interested in
these variables:
...
Boyce continuous index, by setting a minimum threshold of 0.8 for that
...
The total area of projection of the model may include environmental condition which lye outside the range of conditions covered by the calibration dataset. This phenomenon can lead to misinterpretation of the SDM outcomes due to spatial extrapolation. 

Multivariate environmental similarity surfaces (MESS) (Elith et al. 2010) can be used to highlight areas where extrapolation occurs and thus provide details regarding the prediction's uncertainty.
...
We can compute MESS for future predictions in the same way we have done previosuly
...
We can now mask areas of future prediction chatacterised by negative MESS values 
...
@TODO potentially re-do the plots in ggplot? 
...
@TODO maybe a brie mention within this section on the use of area of applicability? 
