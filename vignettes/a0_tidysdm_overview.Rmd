...
Now, we thin the observations to have a one per cell in the raster (it would be better
...
available for the WordClim dataset:
...
We first download the dataset at the right resolution (here 5 arc-minutes):
```{r eval=FALSE}
...
```
We want to choose variables for which presences use a values different from the background (pseudo-absences).
...
And visualise it
```{r autoplot_ens, fig.width=6, fig.height=4}
...
house gas emissions) at the same resolution as the present day data (5 arc-minutes). We first download the data:
```{r eval=FALSE}
...
# train the model
  lacerta_thin_rep_models <-
   lacerta_thin_rep_models %>%
   workflow_map("tune_grid", resamples = lacerta_thin_rep_cv, grid = 10,
                metrics = sdm_metric_set(), verbose = TRUE)
