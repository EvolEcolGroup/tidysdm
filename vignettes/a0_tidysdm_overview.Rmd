...
<https://doi.org/10.15468/dl.srq3b3>.

The dataset is already included in `tidysdm` package:
...
However, we can also easily access and manipulate this data using
`rbgif`. We illustrate here how to do it:
...
GBIF.org (09 April 2024) GBIF Occurrence Download
<https://doi.org/10.15468/dl.yqszft>
...
library(rgbif)
# download file
occ_download_get(key = "0128228-240321170329656", path = tempdir())

# unzip file
unzip(file.path(tempdir(), "0128228-240321170329656.zip"),  overwrite = TRUE,  exdir = tempdir(), unzip = "internal")

#@TODO https://stackoverflow.com/questions/12460938/r-reading-in-a-zip-data-file-without-unzipping-it

# read file
library(readr)
distrib <- data.table::fread(file.path(tempdir(),"0128228-240321170329656.csv"))
#distrib <- read.table(file.path(tempdir(), "0068808-230530130749713.csv"),  row.names = NULL)
distrib <- readr::read_delim(file.path(tempdir(), "0128228-240321170329656.csv"))
...
Now we sample the background. In theory, we could also sample
pseudo-absences (constraining them to be at least 50km away from any
presences and selecting as 3 times as many points as presences) using
`sample_pseudoabs()`. Here, we will use *Lacertidae* records in the
Iberian peninsula. This data is taken from: GBIF Occurrence Download (08
April 2024) <https://doi.org/10.15468/dl.53js5z>.

First download species sampled in the *Lacerta* area, covering 8 genera
of *Lacertidae*:

```{r download_background}
# download presences
library(rgbif)
# download file
...
# unzip file
unzip(file.path(tempdir(), "0121761-240321170329656.zip"),  overwrite = TRUE,  exdir = tempdir(), unzip = "internal")

# read file
library(readr)
backg_distrib <- data.table::fread(file.path(tempdir(),"0121761-240321170329656.csv"))
#backg_distrib <- read.table(file.path(tempdir(), "0121761-240321170329656.csv"),  row.names = NULL)
backg_distrib <- readr::read_delim(file.path(tempdir(), "0121761-240321170329656.csv"))

# keep the necessary columns
lacertidae_background <- backg_distrib[, c("gbifID", "decimalLatitude", "decimalLongitude")]
names(lacertidae_background) <- c("ID", "latitude", "longitude")

lacertidae_background <- st_as_sf(lacertidae_background, coords = c("longitude", "latitude"))
st_crs(lacertidae_background) <- 4326
```

At this point our background species distribution `sf` object needs to
be converted to:
...
Then sample the background, we are choosing here the 'random' sampling,
where background randomly sampled from the region covered by the raster:
...
We start by selecting variables for which presences are markedly
different from the underlying background. First, let's extract climate
for all presences and pseudo-absences:
...
We can use violin plots to compare the distribution of the climatic
variables for presences and pseudo-absences:

```{r fig.height=11, fig.width=7}
lacerta_thin <- na.omit(lacerta_thin)

lacerta_thin %>% plot_pres_vs_bg(class)
```

We want to choose variables for which presences use values different
from the background (pseudo-absences). We can qualitatively look at the
plots, or use a quantitative approach that ranks them based on the
...
non-overlapping distribution between presences and pseudo-absences:

```{r}
vars_overlap <- lacerta_thin %>% dist_pres_vs_bg(class)
names(vars_overlap[vars_overlap > 0.30])

```

These are still a lot of variables. Among them, there are several that
have been argued in the past to be important for the distribution of the
species of interest. Based on this paper
...
@TODO show use of area of applicability
