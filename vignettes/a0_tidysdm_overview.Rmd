...
the Iberian peninsula, *Lacerta schreiberi*. This data is taken from: 
GBIF Occurrence Download (6 July 2023) <https://doi.org/10.15468/dl.srq3b3>. 
...
However, we can also easily access and manipulate this data using `rbgif`.
We illustrate here how to do it:
...
pseudo-absences (constraining them to be at least 50km away
from any presences and selecting as 3 times as many points as presences) using sample_pseudoabs().
Here, we will use *Lacertidae* records in the Iberian peninsula. This data is taken from: 
GBIF Occurrence Download (08 April 2024) <https://doi.org/10.15468/dl.53js5z>. 

First download species sampled in the Lacerta area, covering 8 genera of Lacertidae:
...
At this point our background species distribution sf object needs to be converted
to 
...
```{r climate_variables}
#@TODO we have to drop bio06 here
#suggested_vars <- c("bio05", "bio13", "bio14", "bio15")
```
...
pairs(climate_present[[vars_to_keep]])
```
...
# error
Error in tune::select_best(member, metric) : 
