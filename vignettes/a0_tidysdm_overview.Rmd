...
It is also possible to have further quality checks and filter the
dataset using `CoordinateCleaner v3.0`, this will also help in excluding
outliers (it may take some time, depending on the size of the dataset):

```{r presences_coord_clean, eval = FALSE}
...
saveRDS(lacerta, "../data/lacerta_cleaned.rda")
```

```{r save_and_read, echo=FALSE}
lacerta <- readRDS("../data/lacerta_cleaned.rda")
```
...
We can use violin plots to compare the distribution of the climatic variables for presences and the background:

```{r fig.height=11, fig.width=7}
lacerta_thin %>% plot_pres_vs_bg(class)
```
We want to choose variables for which presences use values different from the background.
We can qualitatively look at the plots, or use a quantitative approach that 
ranks them based on the overlap of the respective density plots:
```{r}
lacerta_thin %>% dist_pres_vs_bg(class)
```

As a first step, we can focus on variables that have at least 30% of 
non-overlapping distribution between presences and background:
...
These are still a lot of variables. Among them, there are several that have been 
argued in the past to be important for the distribution of the species of interest. 
Based on this paper (<https://doi.org/10.1007/s10531-010-9865-2>), we
are interested in these variables:
