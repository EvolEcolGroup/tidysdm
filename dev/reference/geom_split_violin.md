...
This geometry displays the density distribution of two groups side by
side, as two halves of a violin. Note that an empty`x` aesthetic has to
be provided even if you want to plot a single variable (see example
below).
...
- nudge:
  ...
  Add space between the half-violin and the middle of the space allotted
  to a given factor on the x-axis.
